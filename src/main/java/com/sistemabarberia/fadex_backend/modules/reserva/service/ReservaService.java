...
public class ReservaService {
    ...
    public ReservaDTO crearReserva(ReservaRequest request) {
        ...
        switch (rol) {
            ...
            case "ROLE_barbero" -> {
                barbero = barberoRepository.findByPersona_Usuario_IdUsuario(usuario.getIdUsuario()).orElseThrow(() -> new ResourceNotFoundException("Barbero no encontrado"));
                cliente = clienteRepository.findById(request.clienteId()).orElseThrow(() -> new ResourceNotFoundException("Cliente no encontrado"));
            }
            case "ROLE_cliente" -> {
    ...
    private Reserva crearReservaInterna(Cliente cliente, Servicio servicio, Barbero barbero, LocalDate fecha, LocalTime horaInicio, TipoReserva tipo, EstadoReserva estado) {
    ...
    private void validarConflicto(Barbero barbero, LocalDate fecha, LocalTime horaInicio, LocalTime horaFin) {
    ...
    public Page<ReservaDTO> listarReservasPorCliente(Usuario usuario, Pageable pageable) {
        ...
        Cliente cliente = clienteRepository.findByPersona_Usuario_IdUsuario(usuario.getIdUsuario()).orElseThrow(() -> new ResourceNotFoundException("Cliente no encontrado"));
        System.out.println("CLIENTE ID: " + cliente.getClienteId());
        Page<Reserva> reservasPage = reservaRepository.findByCliente_ClienteId(cliente.getClienteId(), pageable);
    ...
    public ResumenDiarioDTO obtenerResumenDiario(Integer barberoId) {
        ...
        List<Reserva> reservas = reservaRepository.findReservasDiarias(barberoId, inicio, fin);
        List<ReservaDTO> dtos = reservas.stream().map(reservaMapper::toDto).collect(Collectors.toList());
        return ResumenDiarioDTO.builder().totalDia(dtos.size()).enEspera(contar(reservas, EstadoReserva.PENDIENTE_PAGO))
                .enProceso(contar(reservas, EstadoReserva.EN_PROCESO)).completados(contar(reservas, EstadoReserva.FINALIZADA)).reservas(dtos).build();
    }
    ...
    public ReservaDTO iniciarAtencion(Long reservaId) {
        ...
        if (reserva.getEstadoReserva() != EstadoReserva.PENDIENTE_PAGO) {
            throw new IllegalStateException("La reserva debe estar en PENDIENTE. Estado actual: " + reserva.getEstadoReserva());
        }
    ...
    public ReservaDTO finalizarAtencion(Long reservaId) {
        ...
        if (reserva.getEstadoReserva() != EstadoReserva.EN_PROCESO) {
            throw new IllegalStateException("La reserva debe estar EN_PROCESO. Estado actual: " + reserva.getEstadoReserva());
        }
        reserva.setEstadoReserva(EstadoReserva.FINALIZADA);
        reserva.setHoraFin(LocalTime.now());
        Reserva guardada = reservaRepository.save(reserva);
        fidelizacionEngine.procesarServicio(guardada);//agregar a fidelizacion enginep para acumular puntos por servicio
        return reservaMapper.toDto(guardada);
    }

    public ResumenSemanalDTO obtenerResumenSemanal(Integer barberoId) {
        LocalDate hoy = LocalDate.now();
        Barbero barbero = barberoRepository.findById(barberoId).orElseThrow(() -> new ResourceNotFoundException("Barbero no encontrado"));
        List<ResumenSemanalDTO.DiaSemana> dias = hoy.minusDays(6).datesUntil(hoy.plusDays(1)).map(fecha -> {
            List<Reserva> reservasDia = reservaRepository.findByBarberoIdAndFechaBetween(barberoId, fecha, fecha);
            long atendidos = contar(reservasDia, EstadoReserva.FINALIZADA);
            long cancelados = contar(reservasDia, EstadoReserva.CANCELADA);
            BigDecimal totalIngresos = reservasDia.stream().filter(r -> r.getEstadoReserva() == EstadoReserva.FINALIZADA).map(Reserva::getTotal)
                    .filter(Objects::nonNull).reduce(BigDecimal.ZERO, BigDecimal::add);
            return new ResumenSemanalDTO.DiaSemana(fecha.toString(), atendidos, cancelados, totalIngresos);}).collect(Collectors.toList());
        BigDecimal ingresosSemana = dias.stream().map(ResumenSemanalDTO.DiaSemana::getTotalIngresos).reduce(BigDecimal.ZERO, BigDecimal::add);
        BigDecimal sueldoBase = barbero.getSueldo();
        BigDecimal comisionSemanal = ingresosSemana.multiply(barbero.getComision().divide(BigDecimal.valueOf(100)));
        BigDecimal totalSemana = sueldoBase.add(comisionSemanal);
        return ResumenSemanalDTO.builder().dias(dias).sueldoBase(sueldoBase).comisionSemanal(comisionSemanal).totalSemana(totalSemana).build();
    }

    private Reserva buscarOFallar(Long id) {
        return reservaRepository.findById(id).orElseThrow(() -> new RuntimeException("Reserva no encontrada: " + id));
    }

    private long contar(List<Reserva> lista, EstadoReserva estado) {
        return lista.stream().filter(r -> r.getEstadoReserva() == estado).count();
    }
    ...
    public List<CitaBarberoResponseDTO> obtenerCitasHoy() {
        ...
        if ("ROLE_admin".equals(rol)) {
            ...
            reservas = reservaRepository.findByBarberoUsernameAndFecha(usuario.getUser(), LocalDate.now());
        } else {
            throw new BusinessException("Acceso denegado", HttpStatus.FORBIDDEN);
        }
        return reservas.stream().map(this::mapToCitaBarberoDTO).toList();
    }

    @Transactional
    public CitaBarberoResponseDTO actualizarEstadoReserva(Long idReserva, ActualizarEstadoReservaDTO dto) {
        if (!securityService.isBarbero()) {
            throw new BusinessException("Acceso denegado: se requiere rol barbero", HttpStatus.FORBIDDEN);
        }
        Usuario usuario = securityService.getUsuarioLogueado();
        Reserva reserva = reservaRepository.findByIdAndBarberoUsername(idReserva, usuario.getUser()).orElseThrow(() -> new ResourceNotFoundException("Reserva no encontrada o no pertenece a este barbero"));
    ...
    private CitaBarberoResponseDTO mapToCitaBarberoDTO(Reserva reserva) {
        ...
        if (reserva.getServicio() != null) {
            ...
            precioServicio = reserva.getServicio().getPrecio() != null ? reserva.getServicio().getPrecio().doubleValue() : 0.0;
        }
        List<DetalleReservaDTO> servicios = List.of(DetalleReservaDTO.builder().nombreCorte(nombreServicio).precio(precioServicio).build());
        return CitaBarberoResponseDTO.builder().idReserva(reserva.getId()).nombreCliente(nombre).apellidoCliente(apellido).telefonoCliente(telefono).fecha(reserva.getFecha())
                .horaInicio(reserva.getHoraInicio()).estado(reserva.getEstadoReserva()).tipoReserva(reserva.getTipoReserva()).servicios(servicios).build();
    }
    ...
    public List<ReservaPendienteDTO> obtenerReservasPendientesDePago() {
        ...
        return reservasSinPago.stream().map(r -> {
            ...
            return ReservaPendienteDTO.builder().id(r.getId()).clienteId(r.getCliente().getClienteId()).clienteNombre(r.getCliente().getPersona().getNombre())
