...
public class RuletaGiroController {
    ...
    private final IRuletaGiroService service;

    @GetMapping
    @PreAuthorize("hasAuthority('RULETA_READ')")
    public ResponseEntity<ApiResponse<PageResponse<RuletaGiroResponseDTO>>> listar(@Valid @ModelAttribute RuletaGiroFiltro filtro, @PageableDefault(size = 10) Pageable pageable) {
        return ResponseEntity.ok(ApiResponse.ok("Giros obtenidos correctamente.", service.listarGiros(filtro, pageable)));
    }

    @GetMapping("/{id}")
    @PreAuthorize("hasAuthority('RULETA_READ')")
    public ResponseEntity<ApiResponse<RuletaGiroResponseDTO>> obtener(@PathVariable Long id) {
        return ResponseEntity.ok(ApiResponse.ok("Giro obtenido correctamente.", service.obtenerGiroPorId(id)));
    }
    ...
    public ResponseEntity<ApiResponse<RuletaGiroResponseDTO>> crear(@Valid @RequestBody RuletaGiroRequestDTO dto) {
        return ResponseEntity.status(HttpStatus.CREATED).body(ApiResponse.ok("Giro registrado correctamente.", service.crearGiro(dto)));
    }
    ...
    public ResponseEntity<ApiResponse<RuletaGiroResponseDTO>> actualizar(@PathVariable Long id, @Valid @RequestBody RuletaGiroRequestDTO dto) {
        return ResponseEntity.ok(ApiResponse.ok("Giro actualizado correctamente.", service.actualizarGiro(id, dto)));
    }

    @DeleteMapping("/{id}")
    @PreAuthorize("hasAuthority('RULETA_MANAGE')")
    public ResponseEntity<ApiResponse<Void>> eliminar(@PathVariable Long id) {
        service.eliminarGiro(id);
